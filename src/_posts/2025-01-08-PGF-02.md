...
In [the last post](/2025/01/01/PGF-01/) we introduced the concept of a
...
Last time we had this function which repeatedly sampled from a given distribution.
...
values, but the `Histogram` object itself. So let's push the idea of composing
distributions one step further, and use the `Select()` method from RandN to
transform the `IDistribution<IEnumerable<double>>` to an
...
does to enumerables; that is, it transforms the type of the distribution given a
function on the underlying types. And, although I've chosen not to here, the
signature of this method means we could use C#'s query syntax instead:
...
generate a normal distribution from two uniformly-distributed values. The
obvious way to write this is with a new class:
...
distribution](/assets/images/2025-01-08-PGF-02/normal.png)

A very satisfying bell curve!

something about 3b1b videos

## Conclusion

We've made a lot of progress so far; these methods for composing and
transforming distributions will be very useful as we go along.

...
Starting with a blank maze (i.e. one with all walls included), we loop through
the cells in the grid, updating the distribution based on the options we have
for each cell. (The `UniformDistribution.TryCreate()` method creates a
distribution which samples uniformly from a collection of values, dealing with

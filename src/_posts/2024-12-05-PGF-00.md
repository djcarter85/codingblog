...
paradigm which centres on the idea of composing functions. As much as possible,
functions in FP should be _pure_, meaning:
...
At first glance it might seem odd to combine procgen and FP. Randomness
is an inherently impure concept; by definition, you don't always get the same
answer when you ask for a random number! However, there are indeed ways of
expressing randomness in a functional way (as we shall see), and I'm intrigued
to see whether we're able to reap the benefits of functional principles within
an intrinsically random system.

...
Now that we've got an implementation of a random walk, we have enough to start
implementing Recursive Backtracker in code.

We need to be able to find out a cell's neighbours, so let's add a little code
to the grid to calculate that.
...
Earlier, when [I defined the algorithm precisely](#the-algorithm), I said:
...
Notice how using the `ImmutableStack` class for the path makes it easy to
implement the "backtrack" step.
...
For the fun of it, I generated a maze on a 25x25 grid.

![Animation of a maze generated using the Recursive Backtracker algorithm on a
larger grid](/assets/images/2025-11-20-PGF-06/maze-animation-big.gif)

I've also updated [the playground website](https://procgenfun.carterdan.net/mazes)
with support for the Recursive Backtracker algorithm.
